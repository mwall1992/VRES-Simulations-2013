...
load('perturbed_gaussian_sims_multi_agent_ghosts.mat');
...
for i = 1:length(stddevs)
    ...
    %% construct mesh
    eval(['mesh = node_positions_0_' num2str(stddevs(i)) '_0_ghosts(:, 1);']);
    ...
    for j = 1:length(timeSteps)
        ...
        for k = 0:realisations-1
            eval(['sol_numeric_' num2str(stddevs(i)) '_' num2str(timeSteps(j)) ' = ' ...
                ...
                'lattice_profile_0_' num2str(stddevs(i)) '_0_' num2str(k) '_' num2str(timeSteps(j)) '_ghosts(:);']);
        end
        eval(['sol_numeric_' num2str(stddevs(i)) '_' num2str(timeSteps(j)) ' = ' ...
    ...
    title(['Average Agent Densities (c0 = 1, std dev = 0.' num2str(stddevs(i)) ', realisations = 1000, Ghosts Model)'], ...
        'FontWeight', 'Bold');
